...
public class SimpleSerialNative implements SimpleSerial
{
    ...
    _initPort( convertToCommString(comPort), 4800, 8, ONESTOPBIT, NOPARITY);
    }
    
    // New a serial port.  
    // Similar to above, but allows greater user configuration
    SimpleSerialNative(int comPort, int baud, 
    ...
    For example, to write the character 'b' to the serial port:  
    ...
    */
    ...
    // For example, to write the string "hello world":  
    ...
    // Read an entire string of data.  
    // If no data available, returns an empty string.
    ...
    System.out.println("--> #### WARNING:  "+
                   "You are reading string data with values less than zero.");
    System.out.println("--> #### This can be dangerous as Char->Byte "+
                   "remapping can change negative values!");
    ...
    // Note that by the time you actually do the reading, 
    ...
    // Close the serial port.  The port is automatically closed on exit, 
    // and you shouldn't normally
    // have to do this yourself.  It's only necessary if you want to close 
    // one port and re-open 
    // another.
    public void close() 
    {
    if (m_Port != 0) 
    ...
    /*
    The following two calls are for more advanced implementations.  
    ...
    dis.mark(512);                      
    // mark this position
    String string = dis.readString();   
    // read a string (or anything)
    try 
    {
    dis.reset();                        
    // set back to location where mark() was called
    }
    catch (IOException e) 
    {}            
    */
    ...
    {
    System.out.println("###ERROR:  You can't get input stream "+
                       "because serial port wasn't opened");
    }
    return null;
    }
    ...
    // You can use these directly if you seek more controll over IO.  
    // Most applications
    // won't use these.
    ...
    // Opens the serial port and returns the port handle.  
    // The port handle is stored for use in the _read/_write calls.
    native int _openSerialPort(String comPort, int baud, int dataBits, 
    ...
    // Reads all available bytes in the serial port buffer.  
    // If nothing available, returns a zero-length array.
    ...
    int                         m_Port = 0;
    Stack                       m_ReadQueue = new Stack();      
    // we're actually using this as a queue, not a stack
    ...
    private void _initPort(String comPort, int baud, int dataBits, 
                           int stopBits, int parity)
    {        
    //System.out.println("Initing NATIVE port.  Com = " + 
    //comPort + ", baud = " + baud);
    ...
    //System.out.println("port no: "+m_Port);
...
class SimpleSerialOutputStream extends OutputStream 
    ...
    if (retVal != 1) 
    {
    System.out.println("### ERROR:  Unkown error writing to serial port.  "+
                       "If persists, probably time to restart Windows!");
    }
    }
}